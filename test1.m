...
for amp_idx = 1:length(amplitudes)
    ...
    for event_idx = 1:num_events_per_amplitude
        ...
        % Na razie weźmy po prostu maksimum jako estymatę amplitudy
        reconstructed_amplitudes_for_current_amp(event_idx) = peak_val;
    end

    % Statystyki rekonstrukcji dla danej amplitudy
    ...
    % Błąd względny rekonstrukcji (średni)
    reconstruction_errors_amplitude(amp_idx) = abs(mean_reconstructed_amplitudes(amp_idx) - current_amplitude) / current_amplitude;
end

%% 8. Wizualizacja wyników rekonstrukcji amplitudy

figure;
errorbar(amplitudes, mean_reconstructed_amplitudes, std_reconstructed_amplitudes, 'o-','LineWidth',1.5);
hold on;
plot(amplitudes, amplitudes, 'k--', 'LineWidth', 1.2); % linia idealna
xlabel('Prawdziwa amplituda [jedn.]');
ylabel('Zrekonstruowana amplituda [jedn.]');
title(sprintf('Rekonstrukcja amplitudy (Fs_{ADC} = %.0f MHz)', Fs_adc_selected / 1e6));
legend('Średnia + std', 'Idealna rekonstrukcja', 'Location', 'NorthWest');
grid on;

figure;
plot(amplitudes, reconstruction_errors_amplitude * 100, 's-r', 'LineWidth', 1.5);
xlabel('Prawdziwa amplituda [jedn.]');
ylabel('Błąd względny rekonstrukcji [%]');
title('Błąd względny rekonstrukcji amplitudy');
grid on;
